...
public class BaseInvoiceEntity  extends BaseEntity {
	...
	public enum DelflagsType{
        ...
        public static Parameter[] getParams(){
            Parameter[] delflagsTypeParams = new Parameter[DelflagsType.values().length];

            for(int i=0;i<delflagsTypeParams.length;i++){
                delflagsTypeParams[i] = new Parameter(DelflagsType.values()[i].getCode().toString(),DelflagsType.values()[i].getName());
            }
