...
//        //企业信息
//        enInfoService.cacheEninfos();
//        //平台信息
//        authCodeInfoService.cacheAuthCodeInfos();
        //缓存税控分类编码
        taxClassificationRedisService.cacheKeyword();
////        taxClassificationService.createTaxClassifiJS();
    ...
    private void initModule() {
        ...
        for (RequestMappingInfo info : map.keySet()) {
            ...
            if (moduleAclResc != null) {
                ...
                AclResource moduleResc = new AclResource(moduleAclResc.id(), moduleAclResc.code(), moduleAclResc.name(), moduleMapping.value().length==0? Arrays.toString(moduleMapping.path()):Arrays.toString(moduleMapping.value()), AclResource.Type.MODULE.getCode(), moduleAclResc.homePage(), moduleAclResc.target().getCode(), moduleAclResc.isMenu());
